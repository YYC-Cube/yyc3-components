...
class FileSystemAPI {
  ...
  async listFiles(): Promise<{
    ...
    return this.request<ProjectFileList>(
      'GET',
      `/api/files/list?projectId=${this.projectId}`
    );
  }

  async readFile(
    filePath: string
  ): Promise<{ ok: boolean; data?: RemoteFileContent; error?: string }> {
    return this.request<RemoteFileContent>('POST', '/api/files/read', {
  ...
  async writeFile(
    ...
    return this.request<{ version: number }>('POST', '/api/files/write', {
      projectId: this.projectId,
      path: filePath,
      content,
      language,
...
export function useFileSync(config?: FileSyncConfig): UseFileSyncReturn {
  ...
  const pushFile = useCallback(
    async (file: VirtualFile): Promise<SyncResult> => {
      const startTime = Date.now();

      if (isBackendAvailable) {
        const res = await apiRef.current.writeFile(
          file.path,
          file.content,
      ...
      // Mock 写入
      const version = await mockApiRef.current.writeFile(
        file.path,
        file.content,
        file.language || 'plaintext'
      );
  ...
  // 批量推送脏文件
  const pushDirtyFiles = useCallback(
    async (vfs: UseVirtualFileSystemReturn): Promise<SyncResult[]> => {
      ...
      setSyncState((prev) => ({
        ...prev,
        status: 'syncing',
        pendingCount: dirtyFiles.length,
      }));
  ...
  const deleteRemoteFile = useCallback(
    async (path: string): Promise<SyncResult> => {
      if (isBackendAvailable) {
  ...
  const pullProject = useCallback(
    async (vfs: UseVirtualFileSystemReturn): Promise<number> => {
