...
function computeDiff(original: string, current: string): DiffLine[] {
  ...
  while (oi < origLines.length || ci < currLines.length) {
    if (
      ...
      oi < origLines.length &&
      (li >= lcs.length || origLines[oi] !== lcs[li])
    ) {
      // Removed line
      origLineNum++;
      result.push({
      ...
      ci < currLines.length &&
      (li >= lcs.length || currLines[ci] !== lcs[li])
    ) {
      // Added line
      currLineNum++;
      result.push({
...
export function DiffViewer({
    ...
    diffLines.forEach((line, i) => {
      if (line.type === 'unchanged') {
        ...
        const nearChange = diffLines
          .slice(Math.max(0, i - 3), Math.min(diffLines.length, i + 4))
  ...
  return (
    <motion.div
      ...
      className="bg-slate-950/98 absolute inset-0 z-50 flex flex-col backdrop-blur-xl"
    >
      {/* Header */}
      <div className="flex flex-none items-center justify-between border-b border-white/[0.06] bg-slate-900/50 px-4 py-2.5">
        ...
        <div className="flex items-center gap-1.5">
          {/* 视图切换 */}
          <div className="flex items-center rounded-lg border border-white/[0.06] bg-black/30 p-0.5">
            <button
              onClick={() => setMode('inline')}
              className={cn(
                'rounded p-1 font-mono text-[10px] transition-all',
                mode === 'inline'
          ...
          {onRevert && !isIdentical && (
            <button
              onClick={onRevert}
              className="flex items-center gap-1 rounded-md border border-red-500/20 bg-red-500/10 px-2 py-1 font-mono text-[10px] text-red-400 transition-all hover:bg-red-500/20"
              title="恢复到原始内容"
            >
      ...
      {/* Footer Stats */}
      {!isIdentical && (
        <div className="flex flex-none items-center justify-between border-t border-white/[0.04] bg-black/30 px-4 py-1.5 font-mono text-[9px] text-slate-600">
          <span>{stats.totalChanges} 处变更</span>
          <span>
            {language} | {originalContent.split('\n').length} →{' '}
        ...
        {lines.map((line: any, idx: number) => {
          if (line.type === 'separator') {
            return (
              <tr key={`sep-${idx}`}>
                <td
                  colSpan={3}
                  className="border-y border-white/[0.03] bg-slate-900/30 px-4 py-1 text-center text-[9px] text-slate-700"
                >
          ...
          return (
            <tr key={idx} className={cn('group', bgClass)}>
              <td className="w-[48px] select-none border-r border-white/[0.03] py-[1px] pr-2 text-right align-top text-[9px] text-slate-700">
                {line.originalLine || ''}
              </td>
              <td className="w-[48px] select-none border-r border-white/[0.03] py-[1px] pr-2 text-right align-top text-[9px] text-slate-700">
                {line.currentLine || ''}
              ...
              <td
                className={cn('whitespace-pre py-[1px] pl-2 pr-4', textClass)}
              >
                <span className="mr-2 select-none text-slate-700">
                  {prefix}
                </span>
                {text}
              </td>
...
function SideBySideDiffView({
  ...
  const displayPairs = useMemo(() => {
    ...
    pairs.forEach((pair, i) => {
      const isChange =
        (pair.left && pair.left.type !== 'unchanged') ||
        (pair.right && pair.right.type !== 'unchanged') ||
        !pair.left ||
        !pair.right;

      if (!isChange && pair.left?.type === 'unchanged') {
        const nearChange = pairs
          .slice(Math.max(0, i - 3), Math.min(pairs.length, i + 4))
          .some(
            (p) =>
              (p.left && p.left.type !== 'unchanged') ||
              (p.right && p.right.type !== 'unchanged') ||
              !p.left ||
              !p.right
          );
        if (nearChange) {
          if (hiddenCount > 0) {
            result.push({ separator: true, count: hiddenCount });
            hiddenCount = 0;
          }
          result.push(pair);
        } else {
  ...
  return (
    <div className="flex w-full">
      ...
      <div className="w-1/2 border-r border-white/[0.06]">
        <div className="sticky top-0 z-10 border-b border-white/[0.04] bg-red-500/[0.06] px-3 py-1 font-mono text-[9px] uppercase tracking-widest text-red-400">
          原始
        </div>
        <table className="w-full border-collapse">
          <tbody>
            {displayPairs.map((pair: any, idx: number) => {
              if (pair.separator) {
                return (
                  <tr key={`sep-l-${idx}`}>
                    <td
                      colSpan={2}
                      className="bg-slate-900/30 py-0.5 text-center text-[9px] text-slate-700"
                    >
                      ··· {pair.count} 行 ···
                    </td>
                  </tr>
              ...
              const bgClass =
                left?.type === 'removed' ? 'bg-red-500/[0.06]' : '';
              return (
                <tr key={`l-${idx}`} className={bgClass}>
                  <td className="w-[40px] select-none border-r border-white/[0.03] py-[1px] pr-1.5 text-right text-[9px] text-slate-700">
                    {left?.originalLine || ''}
                  </td>
                  <td
                    className={cn(
                      'whitespace-pre py-[1px] pl-2 pr-2',
                      left?.type === 'removed'
                        ? 'text-red-400'
                        : 'text-slate-400'
                    )}
      ...
      {/* Right (Current) */}
      <div className="w-1/2">
        <div className="sticky top-0 z-10 border-b border-white/[0.04] bg-emerald-500/[0.06] px-3 py-1 font-mono text-[9px] uppercase tracking-widest text-emerald-400">
          当前
        </div>
        <table className="w-full border-collapse">
          <tbody>
            {displayPairs.map((pair: any, idx: number) => {
              ...
              const bgClass =
                right?.type === 'added' ? 'bg-emerald-500/[0.06]' : '';
              return (
