...
function parseCommand(input: string): ParsedCommand {
  ...
  for (let i = 1; i < tokens.length; i++) {
    if (tokens[i].startsWith('-')) {
      for (const ch of tokens[i].slice(1)) {
        flags[ch] = true;
      }
    } else {
      args.push(tokens[i]);
        ...
        case 'touch': {
          ...
          for (const arg of parsed.args) {
            ...
            if (vfs.exists(fullPath)) {
              addLine(
                'info',
                `touch: '${getFileName(arg)}' 已存在，更新时间戳`
              );
            ...
            if (showAll) {
              entries.forEach((entry) => {
                ...
                if (file) {
                  ...
                  const mod = new Date(file.lastModified).toLocaleTimeString(
                    'zh-CN'
                  );
                  const dirty = file.isDirty ? ' [未保存]' : '';
                  addLine(
                    'output',
                    `${size}B  ${mod}  ${getFileName(entry)}${dirty}`
                  );
                } else {
                  addLine(
                    'output',
        ...
        case 'echo': {
          if (parsed.redirect) {
            ...
            if (parsed.redirect.append) {
              ...
              vfs.writeFile(
                fullPath,
                existing + parsed.redirect.content + '\n',
        ...
        case 'wc': {
          ...
          for (const arg of parsed.args) {
            ...
            if (content === null) {
              ...
              addLine(
                'output',
                `  ${lineCount} 行  ${wordCount} 词  ${charCount} 字符  ${getFileName(arg)}`
              );
            }
          }
          break;
            ...
            searchFiles.forEach((filePath) => {
              ...
              content.split('\n').forEach((line, lineIdx) => {
                if (regex.test(line)) {
                  ...
                  addLine(
                    'output',
                    `${getFileName(filePath)}:${lineIdx + 1}: ${line.trim()}`
                  );
                }
              });
            });
          ...
          switch (subCmd) {
            case 'status': {
              ...
              const newFiles = allFiles.filter(
                (f) => f.lastModified > Date.now() - 300000
              );
              if (newFiles.length > dirtyFiles.length) {
                addLine('output', 'Untracked files:');
                newFiles
                  .filter((f) => !dirtyFiles.find((d) => d.path === f.path))
                  .forEach((f) => {
                    addLine('info', `\t${f.path}`);
                  });
                addLine('output', '');
            ...
            case 'diff': {
              const targetFile = parsed.args[1]
                ? resolvePath(cwd, parsed.args[1])
                : null;
              const dirtyFiles = vfs.getDirtyFiles();
            ...
            case 'commit':
              addLine(
                'success',
                `[main ${Math.random().toString(36).slice(2, 9)}] 虚拟提交成功`
              );
              break;
            default:
              addLine(
                'error',
                `git: '${subCmd}' 不是一个 git 命令（虚拟环境支持: status/log/diff/add/commit）`
              );
          }
          break;
        }
          ...
          switch (subCmd) {
            ...
            case 'i': {
              ...
              if (pkgNames.length === 0) {
                ...
                if (pkgJson) {
                  try {
                    ...
                    deps.forEach(([name, ver]) =>
                      addLine('output', `  + ${name}@${String(ver)}`)
                    );
                    devDeps.forEach(([name, ver]) =>
                      addLine('output', `  + ${name}@${String(ver)} (dev)`)
                    );
                    addLine(
                      'success',
                      `已安装 ${deps.length + devDeps.length} 个依赖`
                    );
                  } catch {
                ...
                const pkgJson =
                  vfs.readFile(pkgJsonPath) ||
                  '{"name":"project","version":"1.0.0","dependencies":{}}';
                try {
                  const pkg = JSON.parse(pkgJson);
                  if (!pkg.dependencies) {
                    pkg.dependencies = {};
                  ...
                  addLine(
                    'success',
                    `已安装 ${pkgNames.length} 个包并更新 package.json`
                  );
                } catch {
            ...
            case 'ls': {
              ...
              if (pkgJson) {
                try {
                  ...
                  Object.entries(pkg.devDependencies || {}).forEach(
                    ([name, ver]) => {
                      addLine('output', `├── ${name}@${String(ver)} (dev)`);
                    }
                  );
            ...
            default:
              addLine(
                'error',
                `npm: '${subCmd}' 命令不支持（虚拟环境支持: install/list/init/run）`
              );
          }
          break;
        }
