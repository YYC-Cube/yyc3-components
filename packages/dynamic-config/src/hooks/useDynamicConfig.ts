...
const MIGRATIONS: Record<number, MigrationFn> = {
  ...
  2: (config: any) => {
    if (!config.computeNodes) {
      config.computeNodes = [];
    }
    if (config.extensions) {
      config.extensions = config.extensions.map((ext: any) => ({
        author: 'Unknown',
        category: 'runtime',
        ...ext,
      }));
    ...
    if (config.endpoints) {
      config.endpoints = config.endpoints.map((ep: any) => ({
    ...
    if (config.agents) {
      config.agents = config.agents.map((a: any) => ({
        nameCN: a.nameCN || a.name || '',
        roleCN: a.roleCN || a.role || '',
        providerId: a.providerId || 'bigmodel',
        ...a,
      }));
    }
    if (config.workflows) {
      config.workflows = config.workflows.map((w: any) => ({
        description: '',
        trigger: 'manual',
        ...w,
      }));
    ...
    if (config.mcpServers) {
      config.mcpServers = config.mcpServers.map((s: any) => ({
        capabilities: [],
        isEnabled: true,
        ...s,
      }));
...
function migrateConfig(config: any): DynamicConfig {
  ...
  while (currentVersion < CURRENT_SCHEMA_VERSION) {
    ...
    if (migration) {
      console.log(
        `[DynamicConfig] Migrating schema v${currentVersion} → v${nextVersion}`
      );
  ...
  return {
    ...
    agents: [
      {
        ...
        color: '#FFD700',
        status: 'active',
        capabilities: ['strategy', 'vision', 'decision'],
        permissions: { canDeploy: true, canReview: true },
      },
      {
        id: 'agent-ca',
        ...
        nameCN: '人类导师',
        version: 'v2.0',
        role: 'Strategic Ethics & Guidance',
        roleCN: '战略伦理与指导',
        modelId: 'glm-4-plus',
        providerId: 'bigmodel',
        color: '#C0C0C0',
        status: 'active',
        capabilities: ['ethics', 'mentoring', 'strategy'],
      ...
      {
        id: 'agent-pulse',
        roleId: 'CENTRAL_PULSE',
        name: 'ZhongShuPulse',
        nameCN: '中枢灵脉',
        version: 'v1.3',
        role: 'Flow Orchestration & Data',
        roleCN: '流程编排与数据',
        modelId: 'glm-4-flash',
        providerId: 'bigmodel',
        color: '#9370DB',
        status: 'standby',
        capabilities: ['orchestration', 'pipeline', 'data'],
        permissions: { canDeploy: false, canReview: false },
      },
      {
        id: 'agent-collab',
        roleId: 'COLLABORATOR',
        name: 'XieZuoMessenger',
        nameCN: '协作使者',
        version: 'v1.0',
        role: 'Cross-Platform Collaboration',
        roleCN: '跨平台协作',
        modelId: 'glm-4-air',
        providerId: 'bigmodel',
        color: '#FF69B4',
    ...
    endpoints: [
      {
        ...
        description: 'Bun Runtime REST API 基地址',
        category: 'api',
        isHealthy: false,
      },
      {
        ...
        envVar: 'VITE_WS_BASE_URL',
        description: 'Bun Runtime WebSocket 基地址',
        category: 'websocket',
        isHealthy: false,
      },
      {
      ...
      },
      {
        ...
        envVar: 'VITE_BIGMODEL_URL',
        description: '智谱 BigModel Z.ai API',
        category: 'llm',
        isHealthy: false,
      },
      {
        ...
        envVar: 'VITE_CHROMA_URL',
        description: 'ChromaDB 向量数据库',
        category: 'vector',
        isHealthy: false,
      },
      {
      ...
      },
      {
        id: 'ep-pg',
        name: 'PostgreSQL',
        url: 'postgresql://localhost:5433/yyc3_family',
        envVar: 'DATABASE_URL',
        description: 'PostgreSQL 15 主数据库',
        category: 'database',
        isHealthy: false,
      },
    ],
    mcpServers: [
      ...
      },
      {
        ...
        url: 'mcp://localhost:3080/mcp/gw',
        status: 'disconnected',
        latency: '-',
        description: 'Google Workspace 集成',
        isEnabled: false,
        capabilities: ['docs', 'sheets', 'drive'],
      },
    ],
    workflows: [
      {
    ...
    extensions: [
      ...
      {
        id: 'ext-dalle',
        name: 'Image_Gen_DALL-E',
        description: '使用 DALL-E 3 API 生成图像',
        isInstalled: false,
        isEnabled: false,
        version: '1.0.0',
        author: 'OpenAI',
      ...
      {
        id: 'ext-chroma',
        name: 'Vector_DB_Chroma',
        description: '本地向量嵌入存储',
        isInstalled: true,
        isEnabled: true,
        version: '0.4.1',
        author: 'Chroma',
...
const DynamicConfigContext = createContext<DynamicConfigContextType | null>(
  null
);
  ...
  try {
    ...
    if (stored) {
      ...
      const merged = {
        ...
        providers: migrated.providers?.length
          ? migrated.providers
          : defaults.providers,
        ...
        endpoints: migrated.endpoints?.length
          ? migrated.endpoints
          : defaults.endpoints,
      ...
      if ((parsed as any)._schemaVersion !== CURRENT_SCHEMA_VERSION) {
        console.log(
          '[DynamicConfig] Migration complete, saving updated config'
        );
        saveConfig(merged);
      }
      return merged;
...
export function DynamicConfigProvider({
  ...
  const mutate = useCallback(
    (updater: (prev: DynamicConfig) => DynamicConfig) => {
      setConfig((prev) => {
  ...
  function makeCRUD<T extends { id: string }>(section: ConfigSection) {
    return {
      ...
      delete: (id: string) =>
        mutate((prev) => ({
          ...prev,
          [section]: (prev[section] as unknown as T[]).filter(
            (item) => item.id !== id
          ),
        })),
    };
  ...
  const exportConfigJSON = useCallback(() => {
    ...
    const safe = {
      ...configRef.current,
      providers: configRef.current.providers.map((p) => ({
        ...p,
  ...
  const importConfigJSON = useCallback((json: string): boolean => {
    try {
      ...
      if (imported.providers && imported.models) {
        setConfig({
          ...imported,
          version: (configRef.current.version || 0) + 1,
          lastModified: Date.now(),
        });
        return true;
      }
...
export function useDynamicConfig(): DynamicConfigContextType {
  ...
  if (!ctx) {
    throw new Error(
      'useDynamicConfig must be used within DynamicConfigProvider'
    );
  }
  return ctx;
}
