...
export interface WorkflowDefinition {
  ...
  steps: Omit<
    WorkflowStep,
    'status' | 'result' | 'error' | 'startedAt' | 'completedAt' | 'retryCount'
  >[];
  ...
  steps: [
    {
      ...
      params: {
        participants: ['PRODUCT_MANAGER', 'CHIEF_ARCHITECT', 'AI_ARCHITECT'],
      },
    ...
    },
    {
      ...
      params: {
        participants: ['AI_ARCHITECT', 'CODE_ARTISAN', 'CHIEF_ARCHITECT'],
      },
      requiresApproval: true,
    },
    {
      ...
      params: {
        participants: ['CODE_ARTISAN', 'AI_ARCHITECT', 'COLLABORATOR'],
      },
    ...
    },
    {
      ...
      params: {
        participants: ['SENTINEL', 'CENTRAL_PULSE', 'CHIEF_ARCHITECT'],
      },
      requiresApproval: false,
    },
  ],
      ...
      steps: [
        {
          id: 'analyze',
          name: '代码分析',
          description: '静态分析与复杂度评估',
          agentId: 'CODE_ARTISAN',
          handler: 'review_analyze',
          params: {},
          requiresApproval: false,
        },
        {
...
for (const stepId of [
  ...
  stepHandlers.set(stepId, (step, ctx) => {
    console.log(
      `[WORKFLOW] Executing step: ${step.name} (${step.id}) | Agent: ${step.agentId || 'system'}`
    );
...
export class WorkflowEngine {
  ...
  constructor() {
    ...
    console.log(
      `[WORKFLOW] Engine initialized: ${this.definitions.size} workflow definitions`
    );
  }
  ...
  trigger(
    ...
    if (running >= wfConfig.maxConcurrent) {
      console.warn(
        `[WORKFLOW] Concurrent limit reached (${running}/${wfConfig.maxConcurrent})`
      );
      return null;
    }
    ...
    console.log(
      `[WORKFLOW] Triggered: ${def.name} (${instance.id.substring(0, 8)})`
    );
  ...
  // ---- Approval ----
  approve(instanceId: string): boolean {
    const instance = this.instances.get(instanceId);
    if (!instance || instance.status !== 'AWAITING_APPROVAL') return false;

    console.log(
      `[WORKFLOW] Approved: ${instance.name} step ${instance.currentStepIndex}`
    );
  ...
  reject(instanceId: string, reason: string): boolean {
    const instance = this.instances.get(instanceId);
    if (!instance || instance.status !== 'AWAITING_APPROVAL') return false;
  ...
  private async executeWorkflow(instance: WorkflowInstance): Promise<void> {
    ...
    while (instance.currentStepIndex < instance.steps.length) {
      ...
      if (
        ...
        wfConfig.creation.requireApproval &&
        step.status === 'PENDING'
      ) {
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () =>
                reject(new Error(`Step timeout: ${wfConfig.stepTimeout}ms`)),
              wfConfig.stepTimeout
            )
          ),
        ]);
        ...
        console.log(
          `[WORKFLOW] Step completed: ${step.name} (${step.completedAt - step.startedAt}ms)`
        );
      } catch (err: any) {
        step.retryCount++;
        if (step.retryCount <= wfConfig.retryMax) {
          console.warn(
            `[WORKFLOW] Step failed, retrying (${step.retryCount}/${wfConfig.retryMax}): ${step.name} — ${err.message}`
          );
      ...
      if (
        !wfConfig.creation.autoAdvance &&
        instance.definitionId === 'creation_pipeline'
      ) {
    ...
    console.log(
      `[WORKFLOW] Completed: ${instance.name} (${duration}ms, ${instance.steps.length} steps)`
    );
  }
