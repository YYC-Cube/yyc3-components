...
export function FileManagerSimple() {
   ...
   */
  const handleDirectoryClick = (node: FSNode) => {
    if (node.type === 'directory') {
      browseDirectory(node.path);
    } else {
  ...
  const handleCreate = () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : '创建失败 / Creation failed'
      );
    }
  };
  ...
  const handleDelete = (node: FSNode) => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : '删除失败 / Deletion failed'
      );
    }
  };
  ...
  return (
    <div className="flex h-full flex-col border border-green-500/30 bg-black p-4 font-mono text-green-500">
      {/* 标题 / Header */}
      <div className="mb-4">
        <h2 className="mb-2 text-xl">FILE MANAGER / 文件管理器</h2>
      ...
      {showCreateForm && (
        <div className="mb-4 flex gap-2">
          <Input
            ...
            className="border-green-500 bg-black text-green-500"
          />
          <Button
      ...
      {/* 当前路径 / Current Path */}
      <div className="mb-2 border border-green-500/30 bg-green-500/5 p-2">
      ...
      {/* 错误提示 / Error Message */}
      {error && (
        <div className="mb-2 border border-red-500 bg-red-500/10 p-2 text-red-500">
          ERROR / 错误: {error}
        </div>
      )}
      ...
      {!loading && (
        <div className="flex-1 overflow-auto border border-green-500/30">
          <table className="w-full">
            <thead className="sticky top-0 bg-green-500/10">
              <tr>
                <th className="border-b border-green-500/30 p-2 text-left">
                  NAME / 名称
                </th>
                <th className="border-b border-green-500/30 p-2 text-left">
                  TYPE / 类型
            ...
            <tbody>
              {files.map((node) => (
                <tr
                  key={node.path}
                  className="cursor-pointer border-b border-green-500/10 hover:bg-green-500/5"
                ...
                >
                  ...
                  <td className="p-2">
                    {new Date(node.modifiedAt).toLocaleString()}
                  </td>
                  <td className="p-2">
                    <Button
              ...
              {files.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-4 text-center text-green-500/50">
                    EMPTY DIRECTORY / 空目录
                  </td>
                </tr>
              )}
      ...
      {/* 状态栏 / Status Bar */}
      <div className="mt-2 border border-green-500/30 bg-green-500/5 p-2 text-sm">
