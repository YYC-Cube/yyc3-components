...
function transformOperation(
  ...
  if (op1.type === 'insert' && op2.type === 'insert') {
    ...
    if (op2.position < op1End && op2End > op1.position) {
      const overlap =
        Math.min(op1End, op2End) - Math.max(op1.position, op2.position);
...
export class CRDTEngine {
    ...
    this.sessions.forEach((session) => {
      session.participants.forEach((participant, userId) => {
        if (participant.ws === ws) {
          ...
          console.log(
            `[CRDT] User ${userId} disconnected from session ${session.sessionId}`
          );
        }
      });
    });
  ...
  private handleJoin(
    ws: any,
    msg: CollabInboundMessage
  ): CollabOutboundMessage[] {
    const { sessionId, documentId, userId, userName } = msg;
    if (!sessionId || !documentId || !userId) {
      return [];
    }
    ...
    console.log(
      `[CRDT] User ${userId} joined session ${sessionId} (${session.participants.size} participants)`
    );
    return responses;
  }
  ...
  private handleOperation(
    ...
    if (baseVersion !== undefined && baseVersion < doc.version) {
      ...
      const missedOps = doc.operationLog.filter(
    ...
    try {
      ...
      if (doc.operationLog.length > this.maxOperationLogSize) {
        doc.operationLog = doc.operationLog.slice(
          -Math.floor(this.maxOperationLogSize / 2)
        );
      }
    ...
    return [
      {
        ...
        participants: targetSession
          ? this.getSerializedParticipants(targetSession)
          : [],
        _targetWs: ws,
      },
    ];
  ...
  getStats() {
    return {
      ...
      onlineParticipants: Array.from(this.sessions.values()).reduce(
        (sum, s) => {
          let online = 0;
